/** Espejo de utils/volumeFromBlocks.js para la Edge Function (Deno). */
...
const MUSCLE_RULES: { keys: string[]; groups: string[] }[] = [
...
export function weekKeyFromYmd(ymd: string): string {
...
function matchesKeyword(normalizedLine: string, key: string): boolean {
...
export function musclesForText(text: string): string[] {
...
function splitExerciseSegments(line: string): string[] {
...
function repsToEffectiveSets(totalReps: number): number {
  return Math.max(1, Math.min(20, Math.round(totalReps / 6)));
}

type WorkPattern = { sets: number; reps: number; effectiveSets: number };

function extractWorkFromSegment(segment: string): WorkPattern[] {
  const raw = String(segment || "");
  const withoutRm = raw.replace(RM_TOKEN_RE, " ");
  const out: WorkPattern[] = [];

  const add = (sets: number, reps: number, effectiveSets = sets) => {
    const s = Number(sets);
    const r = Number(reps);
...
export function extractSetsRepsFromLine(line: string): WorkPattern[] {
  const segments = splitExerciseSegments(line);
  const merged: WorkPattern[] = [];
...
function accumulateVolume(target: Record<string, number>, muscles: string[], effectiveSets: number) {
...
export function analyzeBlocksVolume(blocks: VolumeBlock[]) {
  const muscleTotals: Record<string, number> = {};
  const weeklyMuscle: Record<string, Record<string, number>> = {};
  let parsedLines = 0;
  let unparsedLines = 0;
...
export function formatVolumeAnalysisForPrompt(
  analysis: ReturnType<typeof analyzeBlocksVolume>,
): string {
