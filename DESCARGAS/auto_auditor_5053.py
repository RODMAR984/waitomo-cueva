# ==========================================================
#  auto_auditor.py — auditoria periodica de activos
#
#  - Baja 60-90 dias de velas 5m de Binance (paginado, gratis)
#  - Corre el AUDITOR FINAL portado a Python:
#      5 niveles RSI (25/20/18/15/12), cap 1 dia, x25, SL 3.7%
#      MFE/MAE por nivel, liquidaciones, TP walk-forward
#      Simula la salida DEFINITIVA: 70% al TP medio + 30% trailing 0.5%
#  - Criterios (los validados):
#      nucleo:     MFE/MAE >= 1.5 con casos >= 20 y expectativa > 0
#      ampliacion: MFE/MAE 1.3-1.5 con casos >= 20 y expectativa > 0
#  - Compara contra la lista vigente y arma la PROPUESTA
#  - El bot avisa por Telegram; /aplicar_lista la hace efectiva
# ==========================================================
...
def descargar_velas_5m(symbol, dias=70, pausa=0.25):
        ...
        try:
            r = requests.get(f"{BASE_URL}/fapi/v1/klines",
                             params={"symbol": symbol, "interval": "5m",
                                     "startTime": inicio, "endTime": fin, "limit": limite},
                             timeout=15)
            r.raise_for_status()
            lote = r.json()
        except Exception:
...
def auditar_activo(symbol, dias=70):
    ...
    for nivel in NIVELES:
        ...
        en_caso = False; caso_long = False; caso_ent = 0.0; caso_vel = 0
        caso_mfe = 0.0; caso_mae = 0.0
        n_casos = 0; sum_mfe = 0.0; sum_mae = 0.0; n_liq = 0

        en_tr = False; tr_long = False; tr_ent = 0.0; tr_vel = 0
        tr_tp = 0.0; tr_fase = 0; tr_resto = 1.0; tr_pico = 0.0; tr_neta = 0.0
        t_gana = 0; t_pierde = 0; t_neta_total = 0.0; t_n = 0
        ...
        for i in range(RSI_LEN+2, len(c)):
            ...
            if not en_caso:
                if hay:
                    en_caso = True; caso_long = es_l; caso_ent = c[i]
                    caso_vel = 0; caso_mfe = 0.0; caso_mae = 0.0
                    n_casos += 1
            else:
                caso_vel += 1
                ...
                if caso_vel >= MAX_VELAS:
                    sum_mfe += caso_mfe; sum_mae += caso_mae
                    if caso_mae >= LIQ_PCT:
                        n_liq += 1
                    en_caso = False
            ...
            if not en_tr:
                if hay and listo:
                    en_tr = True; tr_long = es_l; tr_ent = c[i]; tr_vel = 0
                    tr_tp = tp_medio; tr_fase = 0; tr_resto = 1.0
                    tr_pico = 0.0; tr_neta = 0.0
            else:
                tr_vel += 1
        ...
        res = {"symbol": symbol, "nivel": nivel, "casos": n_casos,
               "mfe": round(mfe_p, 2), "mae": round(mae_p, 2), "ratio": round(ratio, 2),
               "liq": round(liq_pct, 0), "winrate": round(wr, 0),
               "expect": round(exp, 1), "tp_pct": round(mfe_p*TP_FRAC_MEDIO, 2),
               "trades": t_n}
...
def clasificar(res):
    """nucleo / ampliacion / descartar segun criterios validados."""
    if res is None:
        return "sin_datos"
    if res["expect"] <= 0:
        return "descartar"
...
def auditar_universo(symbols, dias=70, progreso=None):
    ...
    Audita una lista de symbols. Devuelve:
      {"nucleo": [...], "ampliacion": [...], "descartar": [...], "detalle": {...}}
    `progreso` es un callback opcional (ej: mandar avance por Telegram).
    """
    out = {"nucleo": [], "ampliacion": [], "descartar": [], "detalle": {}}
